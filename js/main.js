...
function getTotalSupply() {
    contract.totalSupply(function(error, result) {
        if (!error) {
            ...
            $('#currentSupply').html(amount + "/10,101");
...
function getCost() {
    contract.cost(function(error, result) {
        if (!error) {
            var amount = result.toNumber();
            cost = amount;
